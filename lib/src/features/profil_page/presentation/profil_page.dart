  ...
  String? _validatePhoneNumber(String? input) {
    ...
    if (!phoneRegex.hasMatch(input)) {
      return AppLocalizations.of(context)?.invalidPhoneNumberError ??
          'Ungültige Telefonnummer (min. 8 Zeichen, erlaubt: Ziffern, Leerzeichen, +, -, (, ), .)';
    }
    return null;
  }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                "${AppLocalizations.of(context)?.profilePictureUpdateError ?? "Fehler beim Aktualisieren deines Profilbilds:"} $e"),
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  "${AppLocalizations.of(context)?.profileInfoSaveError ?? "Fehler beim Speichern deiner Profilinformationen:"} $e"),
              backgroundColor: AppColors.famkaRed,
            ),
          );
        }
                                      ...
                                      onTap: () {
                                        ...
                                        if (phoneNumber.isNotEmpty) {
                                          ...
                                          ScaffoldMessenger.of(context)
                                            ...
                                            const SnackBar(
                                              content: Text(
                                                  'Bitte gib zuerst eine Telefonnummer ein'),
                                            ),
                                          );
                                        }
                                      },
                                      child: const Tooltip(
                                        message: 'Zum Anrufen tippen',
